...
public class NettyClientTwo {
            ...
            b.group(group).channel(NioSocketChannel.class)
                    ...
                    .handler(new ChannelInitializer<SocketChannel>() {
                        protected void initChannel(SocketChannel ch) throws Exception {
                            //设置超时时间
                            ch.pipeline().addLast(new IdleStateHandler(300,300,600,TimeUnit.SECONDS));
                            //这里设置通过增加包头表示报文长度来避免粘包
                            ch.pipeline().addLast(new LengthFieldBasedFrameDecoder(1024, 0, 2,0,2));                            ch.pipeline().addLast("msgpack decoder",new MessagePackDecoder());
                            //增加解码器
    ...
    public static void main(String[] args) {
        new NettyClientTwo().connect(NettyConstants.LOCAL_PORT, NettyConstants.LOCAL_IP);
